...
def create_app(test_config=None):
  ...
  def search_questions():
    form = request.get_json()
    term = form.get('searchTerm', None)
    if term:
      search_results = Question.query.order_by(Question.id).filter(Question.question.ilike('%{}%'.format(term)))
      page = request.args.get('page', 1, type=int)
      first = (page - 1) * 10
      last = first + 10
      questions = [question.format() for question in search_results]
      search_questions = questions[first:last]
